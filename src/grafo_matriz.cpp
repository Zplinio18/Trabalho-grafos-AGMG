 ...
 */
grafo_matriz::grafo_matriz() : matriz(nullptr), matriz_inicializada(false), nos(nullptr) {
    num_nos = 0;
}
...
no_grafo* grafo_matriz::get_no(int id) {
    if (id < 1 || id > num_nos || !nos) return nullptr;
    return nos[id - 1];
}
...
void grafo_matriz::add_no(int id, int peso) {
    if (id < 1 || id > num_nos) return;

    if (!nos) {
        nos = new no_grafo*[num_nos];
        for (int i = 0; i < num_nos; ++i) {
            nos[i] = nullptr;
        }
    }
...
void grafo_matriz::add_aresta(int origem, int destino, int peso) {
    ...
    if (!get_no(origem) || !get_no(destino)) return;
